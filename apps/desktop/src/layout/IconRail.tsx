...
import { IconHome, IconGraph, IconAudit, IconLogs, IconInbox, IconCost, IconMarkdown, IconSettings } from "./icons.js";
import { useT } from "../i18n/index.js";

export type RailKey = "home" | "canvas" | "audit" | "logs" | "inbox" | "cost" | "mdviewer" | "feishu" | "settings";
 ...
 */
export function IconRail({ homeOpen, canvasOpen, tab, settingsOpen, costOpen, inboxBadge, onAction }: Props) {
  const t = useT();
  return (
    <nav className="railbar">
      {/* 欢迎主页：软件总览 + 各界面入口；随时点回来 */}
      <RailButton title={t("Home · 欢迎主页（总览与各界面入口）")} active={homeOpen} onClick={() => onAction("home")}>
        <IconHome />
      </RailButton>
      <div className="railbar-sep" />
      {/* 主视图：节点图（画布）。终端/转录靠左侧会话列表进入 */}
      <RailButton title={t("节点图（展开/收起连线画布）")} active={!homeOpen && canvasOpen} onClick={() => onAction("canvas")}>
        <IconGraph />
      </RailButton>
      <div className="railbar-sep" />
      {/* 全局面板：收件箱 / 审计 / 服务日志 */}
      <RailButton
        ...
        active={!homeOpen && tab === "inbox"}
        badge={inboxBadge}
        onClick={() => onAction("inbox")}
      >
        <IconInbox />
      </RailButton>
      <RailButton title={t("审计 · 谁问了什么")} active={!homeOpen && tab === "audit"} onClick={() => onAction("audit")}>
        <IconAudit />
      </RailButton>
      <RailButton title={t("服务日志")} active={!homeOpen && tab === "logs"} onClick={() => onAction("logs")}>
        <IconLogs />
      </RailButton>
